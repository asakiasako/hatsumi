...
<header id="header">
        ...
        <?php
            $logo = yukimoe_get_option( 'logo' );
            $logo = $logo ? $logo : yukimoe_static('image/logo.png');
			$logo_sm = yukimoe_get_option( 'logo-small' );
            $logo_sm = $logo_sm ? $logo_sm : yukimoe_static('image/logo-small.png');
        ?>
        ...
        <div class="search right">
            <form class="search-form clearfix" method="get" action="<?php bloginfo('home');?>/">
                <label for="top-search"><i class="yukimoe">&#xe618;</i></label>
